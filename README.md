...
Round 2026-07-27. Because Vistra publishes no API, the artifacts here are evidence of absence plus the posture of the properties it does run — no OpenAPI, packages, MCP, scopes, authentication, errors, lifecycle, sandbox, changelog, CLI, components or skills artifacts were created, because there is nothing real to ground them in.

- [`conformance/vistra-energy-conformance.yml`](conformance/vistra-energy-conformance.yml) — 19 standards checked (Green Button/ESPI, NAESB REQ.21, IEEE 2030.5, OpenADR, OCPP, OCPI, IEC CIM, OAuth 2.0, OIDC, RFC 9457, RFC 9116, RFC 8414, plus DNS/TLS hygiene). Every API-facing standard is `conforms: false` on evidence of absence; Green Button is `claimed: association-only` via the US DOE commitment list.
- [`security/vistra-energy-domain-security.yml`](security/vistra-energy-domain-security.yml) — TLS, HSTS, DNSSEC, CAA, SPF and DMARC probed across 10 Vistra hosts and 7 registrable domains. All HTTPS; zero DNSSEC; zero CAA; SPF everywhere; every DMARC record is `p=none`; `www.vistracorp.com`, `www.txu.com` and `www.trieagleenergy.com` return no HSTS header.
- [`well-known/vistra-energy-well-known.yml`](well-known/vistra-energy-well-known.yml) — every `/.well-known/` and discovery path probed on all 10 hosts. Zero documents found; no `WellKnown` or `SecurityTxt` pointer is wired, and `services.txu.com` soft-200s (returns the MyAccount login HTML for `/openapi.json`, `/swagger/v1/swagger.json` and `/.well-known/openid-configuration`).
- [`llms/vistra-energy-llms.txt`](llms/vistra-energy-llms.txt) — generated by API Evangelist (Vistra serves no `/llms.txt`), leading with the API posture and the `vistra.com` homonym warning so an agent does not attribute the wrong company's developer portal.

No vulnerability-disclosure or trust-center artifact was written: `probe-security-programs.py` returned `vdp=none trust=none`, `/security`, `/responsible-disclosure` and `/vulnerability-disclosure` all 404, `trust.vistracorp.com` does not resolve, and the 51-page corporate sitemap has no security or trust page. No first-party client library exists on npm, PyPI or RubyGems.
